export const TRAIN_CLASSES = [
  { label: 'Sleeper (SL)', value: 'SL' },
  { label: 'AC 3 Tier (3A)', value: '3A' },
  { label: 'AC 2 Tier (2A)', value: '2A' },
  { label: 'AC First Class (1A)', value: '1A' },
  { label: 'Second Sitting (2S)', value: '2S' },
  { label: 'AC Chair Car (CC)', value: 'CC' },
  { label: 'General (GN)', value: 'GN' },
];

export const QUOTAS = [
  { label: 'General (GN)', value: 'GN' },
  { label: 'Tatkal (TQ)', value: 'TQ' },
  { label: 'Ladies (LD)', value: 'LD' },
  { label: 'Premium Tatkal (PT)', value: 'PT' },
  { label: 'Lower Berth/Sr. Citizen (SS)', value: 'SS' },
];

export const BOOKING_STATUSES = [
  { label: 'CNF (Confirmed)', value: 'CNF' },
  { label: 'RAC (Reservation Against Cancellation)', value: 'RAC' },
  { label: 'WL (Waiting List)', value: 'WL' },
  { label: 'CAN (Cancelled)', value: 'CAN' },
];

export const GENDERS = [
  { label: 'Male', value: 'Male' },
  { label: 'Female', value: 'Female' },
  { label: 'Transgender', value: 'Transgender' },
];

export const CONVENIENCE_FEE = 23.60;
